...
THREAT_KEYWORDS: Dict[str, Tuple[str, float]] = {
    ...
    # theft / stealing — ONLY unambiguous terms.
    # Removed: "grab", "pick up", "picks up", "grabbed", "grabbing" — too generic.
    # These fire on assault descriptions ("grabbed and pushed", "picking up fallen person").
    # Only keep terms that are unambiguously about taking property from a surface/store.
    "steal": ("theft", 0.85),
    "stealing": ("theft", 0.90),
    "stolen": ("theft", 0.90),
    "theft": ("theft", 0.90),
    "thief": ("theft", 0.90),
    "shoplift": ("theft", 0.95),
    "shoplifting": ("theft", 0.95),
    "pickpocket": ("theft", 0.90),
    "snatch": ("theft", 0.80),
    "snatching": ("theft", 0.85),
    "pocketing": ("theft", 0.80),
    "concealing merchandise": ("theft", 0.90),
    "concealing item": ("theft", 0.90),
    "concealing object": ("theft", 0.90),
    "slips into pocket": ("theft", 0.90),
    "places into pocket": ("theft", 0.85),
    "hides item": ("theft", 0.85),
    "takes item": ("theft", 0.80),
    "takes object": ("theft", 0.80),
    "removes item from shelf": ("theft", 0.90),
    "removes item from table": ("theft", 0.90),
    "object disappears": ("theft", 0.85),
    "item disappears": ("theft", 0.85),
    "item missing": ("theft", 0.80),
    "without paying": ("theft", 0.95),
...
def label_from_signals(
    ...
    if theft_strength > 0.0:
        label = "shoplifting_detected" if theft_strength >= 0.90 else "theft_detected"
        _add_hypothesis(
            acc,
            label,
            0.60 + min(theft_strength, 2.0) * 0.18,
        )
