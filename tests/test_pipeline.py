...
def test_extract_caption_threats():
    tags = extract_caption_threats("Two people fight and one appears to fall near an intruder")
    assert "physical_altercation" in tags
    assert "person_fallen" in tags
    assert "intruder_detected" in tags


def test_label_from_signals():
    signals = {"contact_score": 0.8, "peak_motion": 0.9, "after_hours": 0}
...
def test_compute_alert_score_safe_cap():
    score, stage, _ = compute_alert_score(
        "handshake",
        {"contact_score": 0.9, "peak_motion": 0.9, "people_count": 3, "vehicle_count": 0, "after_hours": 0},
        ["handshake"],
    )
...
def test_compute_alert_score_critical():
    score, stage, _ = compute_alert_score(
        "weapon_detected",
        {"contact_score": 0.2, "peak_motion": 0.3, "people_count": 1, "vehicle_count": 0, "after_hours": 0},
        ["weapon_detected"],
    )
    assert score >= 0.7
    assert stage == "C"
